...
from src.XML_extraction import HeartRate
from src.XML_extraction import BloodOxygen
...
def convert_to_array(data, times):
    ...
    for i in range(len(data)):
        object = data[i]
        obj_time = object.startDate + (object.editDate - object.startDate) / 2
        related_time = get_obj_time(obj_time, times)
        if related_time is not None:
            if isinstance(object, HeartRate):
...
def create_second_by_second_data(data, mood_times):
    ...
    for i in range(len(data)):
        if data[i][2] == 'HR':
            hr_second_by_second.append([data[i][0].timestamp(), data[i][3]])
        else:
            o2_second_by_second.append([data[i][0].timestamp(), data[i][3]])
    ...
    while i < len(hr_second_by_second):
        ...
        next = hr_second_by_second[i + 1] if i < len(hr_second_by_second) - 1 else current
        ...
        if next[0] - current[0] > 60 or i == len(hr_second_by_second) - 1:
            ...
            for adder in range(two_min_subset[0][0] + 1, two_min_subset[-1][0]):
                if adder not in sub[:, 0]:
                    hr_second_by_second.append([adder, f(adder)])
    ...
    i = 0
    while i < len(o2_second_by_second):
        current = o2_second_by_second[i]
        prev = o2_second_by_second[i - 1] if i > 0 else current
        next = o2_second_by_second[i + 1] if i < len(o2_second_by_second) - 1 else current

        # check if next data point is more than 1 minute away or if we are at the end of the data
        if next[0] - current[0] > 60 or i == len(o2_second_by_second) - 1:
            i += 1
        elif next[0] - current[0] == 1:
            # check if we already have data for the next second
            i += 1
            ...
            two_min_subset = [current]
            cursor = i + 1
            while cursor < len(o2_second_by_second) and o2_second_by_second[cursor][0] - current_time < 120:
                two_min_subset.append(o2_second_by_second[cursor])
                cursor += 1
            ...
            for adder in range(two_min_subset[0][0] + 1, two_min_subset[-1][0]):
                if adder not in sub[:, 0]:
                    o2_second_by_second.append([adder, f(adder)])

            sub_end = two_min_subset[-1][0]
            # move i to the end of the subset + 1
            while i < len(o2_second_by_second) and o2_second_by_second[i][0] <= sub_end:
                i += 1
...
class DataCreator:
    ...
    def __init__(self, data, mood_times):
        self.data = convert_to_array(data, mood_times)
        self.mood_times = mood_times
